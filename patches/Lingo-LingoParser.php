...
class LingoParser {
	...
	private function shouldParse( Parser $parser ): bool {
		...
		// Parser::$mOutput became a typed property in MediaWiki 1.46, so reading
		// it before a parse has begun raises an Error instead of returning null.
		// The original "!$parser->getOutput()" test was written for the old
		// untyped property, which means the guard intended to detect "no output
		// yet" is itself what throws:
		//
		//   Typed property MediaWiki\Parser\Parser::$mOutput must not be
		//   accessed before initialization
		//
		// Any request reaching this hook with a Parser that has not started
		// parsing dies with that error. Editing is the visible casualty - it
		// took out saving and previewing on a live wiki - while plain page
		// views, which always parse, are unaffected.
		//
		// Catching Error preserves the original intent: no parser output means
		// there is nothing to annotate, so skip.
		try {
			$parserOutput = $parser->getOutput();
		} catch ( \Error $e ) {
			return false;
		...
		if ( isset( $parserOutput->getPageProperties()['noglossary'] ) ) { // __NOGLOSSARY__ found in wikitext
			return false;
		}
